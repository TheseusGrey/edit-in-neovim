...
export class Windows extends Host {
  ...
  configureHostArgs(neovimPath: string, defaults: SpawnProcessOptions): SpawnProcessOptions {
    const terminalName = this.hostBinary?.split('\\').pop()?.toLowerCase() || '';
    ...
    switch (terminalName) {
      case "wt.exe":
        ...
        return {
          ...defaults,
          spawnArgs: ['new-tab', '--title', 'Neovim', neovimPath, '--listen', this.settings.listenOn],
        };
      case "cmd.exe":
        console.debug(`edit-in-neovim:\nProcess spawn config for ${terminalName}: ${JSON.stringify(defaults, null, 2)}`)
        return {
          ...defaults,
          spawnArgs: ['/c', 'start', `"Neovim"`, `"${neovimPath}"`, '--listen', this.settings.listenOn],
          shell: true,
        };
      case "powershell.exe":
      case "pwsh.exe":
  ...
  getSearchPaths(): Set<string> {
    ...
    if (LOCALAPPDATA) {
      ...
      paths.add(Windows.normalizePath(`${LOCALAPPDATA}/Microsoft/WinGet/Packages`));
    }
    if (PROGRAMFILES) {
